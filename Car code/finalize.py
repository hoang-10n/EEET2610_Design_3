...
def findCube(rgb): #object detection
    ...
    for cont in contours: #sobel edge detection
...
def PowerConsumption(): #Meausuring the power consumption
    SHUNT_OHMS = 0.1
...
def RunCar():
    ...
    def checkFrontWall2(): #Wall detection
        frontDistance = readRangeFront()
        if (GPIO.input(IRFRONT) == GPIO.LOW):
    #         forward(15)
            motorStop()
    ...
    def checkGate(): #obstacle detection 2
...
#Manual control
    def autonomy():
        try:
            ...
            while (auto_mode):
                if (readRangeFront() < 20 or checkIRFront()):
                    publish.single("Design3/Wall", "f", hostname = BROKER_LOCATION)
                elif (readRangeRight() < 20 or checkIRRightFront()):
                ...
                if (object_passed == 2):
                    global gate_time
                    if (timedelta(seconds=time.time() - gate_time).total_seconds() > 6):
                        while(True):
                            global detecting
                            detecting = True
                            motorStop()
